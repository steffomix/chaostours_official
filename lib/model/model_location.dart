...
enum LocationPrivacy {
  ...
  normal(1, Color.fromARGB(255, 0, 166, 0)),

  /// send notification
  private(2, Color.fromARGB(255, 0, 0, 155)),

  /// do nothing
  restricted(3, Color.fromARGB(255, 155, 0, 155)),

  /// record without location if enabled by user settings
  none(4, Color.fromARGB(255, 0, 0, 0)); // no location found

  static final Logger logger = Logger.logger<LocationPrivacy>();
  final int level;
  final Color color;
  const LocationPrivacy(this.level, this.color);
  static final int _saveId = LocationPrivacy.private.level;
  ...
  static LocationPrivacy byValue(int id) {
    try {
      ...
      return LocationPrivacy.private;
    }
  }
}
